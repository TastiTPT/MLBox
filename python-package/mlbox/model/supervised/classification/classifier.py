    ...
    **params : parameters of the corresponding classifier.
        ...
        if 'strategy' in params.keys():
            ...
            for k,v in self.__classif_params.items():
                if k not in self.get_params().keys():
                    ...
                    ret = setattr(self.__classifier,k,v)
    ...
    def __set_classifier(self, strategy):
        ...
        if(strategy == 'RandomForest'):
            ...
            self.__classifier = XGBClassifier(n_estimators=500, max_depth=6, learning_rate=0.05, colsample_bytree=0.8, colsample_bylevel=1., subsample=0.9, nthread=-1, seed=0)
            ...
            self.__classifier = BaggingClassifier(base_estimator=None, n_estimators=500, max_samples=.9, max_features=.85, bootstrap=False, bootstrap_features=False, n_jobs=-1, random_state=0)

        elif(strategy == "AdaBoost"):
            self.__classifier = AdaBoostClassifier(base_estimator=None, n_estimators=400, learning_rate=.05, algorithm='SAMME.R', random_state=0)

        elif(strategy == "Linear"):
            self.__classifier = LogisticRegression(penalty='l2', dual=False, tol=0.0001, C=1.0, fit_intercept=True, intercept_scaling=1, class_weight=None, random_state=0, solver='liblinear', max_iter=100, multi_class='ovr', verbose=0, warm_start=False, n_jobs=-1)
    ...
    def fit(self, df_train, y_train):
        ...
        ### sanity checks
        if (type(df_train) != pd.core.frame.DataFrame):
        ...
        if self.__fitOK:
            ...
            if (self.get_params()["strategy"] in ["Linear"]):
                ...
                for i, col in enumerate(self.__col):
                    importance[col] = np.mean(np.abs(self.get_estimator().coef_), axis=0)[i]
                ...
                try:
                    ...
                    f = sum(weight * np.mean(np.abs(est.coef_), axis=0) for weight, est in zip( self.get_estimator().estimator_weights_, self.get_estimator().estimators_))/norm   #Linear
                ...
                for i, b in enumerate(self.get_estimator().estimators_):
                    ...
                    try:
                        ...
                        f = np.mean(np.abs(b.coef_), axis=0)   #Linear
